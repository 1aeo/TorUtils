...
CONTROL_PORT_START=""  # Required parameter - must be specified by user
METRICS_PORT_START=""  # Required parameter - must be specified by user
...
${YELLOW}GLOBAL OPTIONS:${NC}
    ...
    --control-port-start NUM  Starting ControlPort number (REQUIRED)
    --metrics-port-start NUM  Starting MetricsPort number (REQUIRED)

${YELLOW}EXAMPLES:${NC}
    # Complete migration workflow
    $0 create-inputs --control-port-start 47100 --metrics-port-start 31000
    $0 migrate --control-port-start 47100 --metrics-port-start 31000 --verbose
    $0 manage start --parallel
    $0 list
    
    # Generate master list from deployed relays (no migration files needed)
    $0 list --deployed --verbose
    
    # Create new relays with fresh keys
    $0 create-new --nicknames new_relays.txt --ip-file new_ips.txt --control-port-start 47200 --metrics-port-start 31100 --verbose
...
${YELLOW}OPTIONS:${NC}
    ...
    --existing FILE      Source relay names file (alternative to --input-dir)
    --new FILE           Target relay names file (default: ${OUTPUT_RELAYS})
    --output-file FILE   Target relay names file (alias for --new)
...
1. Directory mode: Reads old relay names from directory structure (--input-dir)
2. File mode: Reads old relay names from a text file (--existing)

${YELLOW}EXAMPLES:${NC}
    # Directory mode (existing behavior)
    $0 create-inputs --network 64.65.1.0/24 --octets 1,2,3,4,5 --control-port-start 47100 --metrics-port-start 31000
    $0 create-inputs --input-dir old_names_dir/ --new new_names.txt
    
    # File mode (new feature)
    $0 create-inputs --existing old_relay_names.txt --new new_relay_names.txt
    $0 create-inputs --existing old_relays.txt --new new_relays.txt --network 64.65.1.0/24 --octets 1,2,3 --control-port-start 47100 --metrics-port-start 31000
...
${YELLOW}EXAMPLES:${NC}
    $0 migrate --control-port-start 47100 --metrics-port-start 31000 --verbose --parallel
...
${YELLOW}OPTIONS:${NC}
    ...
    --deployed          Generate list from deployed relays in /etc/tor/instances (no migration files needed)
    --verbose           Show detailed output
    --help              Show this help

${YELLOW}DESCRIPTION:${NC}
By default, this command uses migration files (batch_migration.txt, ipv4_addresses.txt).
Use --deployed to generate the list directly from already deployed relays.

${YELLOW}EXAMPLES:${NC}
    # From migration files (default)
    $0 list --format csv --output relays.csv
    $0 list --sort ip
    
    # From deployed relays (no migration files needed)
    $0 list --deployed --format csv --output deployed_relays.csv
    $0 list --deployed --sort ip --verbose
...
create_inputs() {
    ...
    local metrics_port_specified=false
    local control_port_specified=false
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --control-port-start)
                ...
                control_port_specified=true
                shift 2
                ;;
            --metrics-port-start)
                metrics_port_start="$2"
                metrics_port_specified=true
                shift 2
                ;;
            --help)
    ...
    if [ "$use_file_mode" = true ]; then
        # File mode: Read old relay names from file
        if [ ! -f "$existing_file" ]; then
            print_error "Existing relay names file not found: $existing_file"
            exit 1
        fi
        
        print_info "Reading relay names from file $existing_file..."
        
        # Read relay names from file (skip comments and empty lines)
        grep -v '^[[:space:]]*#' "$existing_file" | grep -v '^[[:space:]]*$' | sort > "$temp_input_file"
        
        local input_count=$(wc -l < "$temp_input_file")
        print_info "Found $input_count relay names in file"
        
    else
        # Directory mode: Read old relay names from directory structure (existing behavior)
        if [ ! -d "$input_dir" ]; then
    ...
    if [ ! -f "$output_file" ]; then
        print_error "New relay names file not found: $output_file"
        exit 1
    fi
    ...
    if [ "$use_file_mode" = true ]; then
        print_info "Creating batch migration file from $existing_file and $output_file..."
    else
    ...
    if [ -n "$network" ] && [ -n "$octets" ]; then
        if [ "$control_port_specified" = false ]; then
            print_error "ControlPort starting number is required when creating IP addresses file"
            print_info "Please specify --control-port-start NUM"
            exit 1
        fi
        
        if [ "$metrics_port_specified" = false ]; then
            print_error "MetricsPort starting number is required when creating IP addresses file"
            print_info "Please specify --metrics-port-start NUM"
            exit 1
        fi
...
migrate_torrc() {
    ...
    if [ "$control_port_specified" = false ]; then
        print_error "ControlPort starting number is required for torrc migration"
        print_info "Please specify --control-port-start NUM"
        return 1
    fi
    
    if [ "$metrics_port_specified" = false ]; then
        print_error "MetricsPort starting number is required for torrc migration"
        print_info "Please specify --metrics-port-start NUM"
        return 1
    fi
...
migrate_relays() {
    ...
    # Check ControlPort is specified when torrc migration is needed
    if [ "$instances_only" = false ] && [ "$control_port_specified" = false ]; then
        print_error "ControlPort starting number is required for torrc migration"
        print_info "Please specify --control-port-start NUM"
        exit 1
    fi
    
    # Check MetricsPort is specified when torrc migration is needed
    if [ "$instances_only" = false ] && [ "$metrics_port_specified" = false ]; then
        print_error "MetricsPort starting number is required for torrc migration"
        print_info "Please specify --metrics-port-start NUM"
        exit 1
    fi
...
EOF
    ...
    # Count data lines, excluding headers based on format
    local count
    case "$format" in
        csv)
            # For CSV, skip the header line
            count=$(tail -n +2 "$output" | wc -l 2>/dev/null || echo "0")
            ;;
        json)
            # For JSON, count the relay objects
            count=$(grep -c '"nickname":' "$output" 2>/dev/null || echo "0")
            ;;
        txt)
            # For TXT, skip comment lines starting with #
...
generate_master_list() {
    ...
    # Handle deployed mode - scan existing relays directly
    if [ "$deployed_mode" = true ]; then
        ...
        for relay_name in "${deployed_relays[@]}"; do
            ...
            # Get MetricsPort (extract port number after colon)
            metrics_port=$(sudo grep "^MetricsPort" "$torrc_file" 2>/dev/null | head -1 | awk '{print $2}' | cut -d':' -f2)
            if [ -z "$metrics_port" ]; then
                metrics_port="N/A"
            fi
        ...
        case "$format" in
            txt)
                {
                    echo "# Tor Relay Master List (from deployed relays)"
                    echo "# Generated: $(date)"
                    echo "# Source: /etc/tor/instances/ and /var/lib/tor-instances/"
                    echo "# Format: Nickname | Fingerprint | IP Address | ControlPort | MetricsPort"
                    echo "#"
            ...
            json)
                {
                    echo "{"
                    echo "  \"generated\": \"$(date -Iseconds)\","
                    echo "  \"source\": \"Deployed relays from /etc/tor/instances/\","
                    echo "  \"relays\": ["
                    local first=true
                    while IFS='|' read -r nickname fingerprint ip control_port metrics_port; do
                        if [ "$first" = true ]; then
    ...
    # Default mode - use migration files
    if [ ! -f "$BATCH_FILE" ] || [ ! -f "$IP_FILE" ]; then
        ...
        print_info "Use --deployed option to generate list from already deployed relays"
        exit 1
    fi
...
${YELLOW}EXAMPLES:${NC}
    $0 create-new --nicknames new_relays.txt --ip-file new_ips.txt --control-port-start 47200 --metrics-port-start 31100 --verbose
    $0 create-new --nicknames new_relays.txt --dry-run --control-port-start 47300 --metrics-port-start 31200
...
EOF
    ...
    # Check ControlPort is specified
    if [ "$control_port_specified" = false ]; then
        print_error "ControlPort starting number is required for creating new relays"
        print_info "Please specify --control-port-start NUM"
        exit 1
    fi
    
    # Check MetricsPort is specified
    if [ "$metrics_port_specified" = false ]; then
        print_error "MetricsPort starting number is required for creating new relays"
        print_info "Please specify --metrics-port-start NUM"
        exit 1
    fi
    ...
    if [ "$manual_ports" = true ]; then
        ...
        # Since MetricsPort is now required, this case should not happen
        # But we keep the logic for backward compatibility in case ControlPort is auto-calculated
        next_control_port=$((max_control_port + 1))
        next_metrics_port="$metrics_port_start"  # Always use the required MetricsPort
        
        # If no existing relays, use defaults for control port
        if [ "$max_control_port" -eq 0 ]; then
            next_control_port="$CONTROL_PORT_START"
        fi
        ...
        print_info "Found $existing_count existing relay configurations"
        print_info "Highest ControlPort: $max_control_port, starting new relays at: $next_control_port"
        print_info "Using specified MetricsPort starting at: $next_metrics_port"
    fi
...
main() {
    ...
    case "$command" in
        ...
        migrate-torrc)
            ...
            # Check that ControlPort is specified
            if [ -z "$CONTROL_PORT_START" ]; then
                print_error "ControlPort starting number is required for torrc migration"
                print_info "Please specify --control-port-start NUM"
                exit 1
            fi
            # Check that MetricsPort is specified
            if [ -z "$METRICS_PORT_START" ]; then
                print_error "MetricsPort starting number is required for torrc migration"
                print_info "Please specify --metrics-port-start NUM"
                exit 1
            fi
